...
<div class="container p-6 mx-auto">
    ...
    <div class="mb-4 bg-white rounded-sm shadow-sm">
        <div class="flex items-center justify-between p-6">
            <div>
                <h1 class="text-xl font-bold text-purple-800 font-heading">Daftar Peserta</h1>
                <p class="mt-1 text-sm text-gray-600">Kelola informasi dan data peserta</p>
            </div>
            <nav class="flex space-x-1 font-sans text-xs text-gray-500">
                ...
                <span>/</span>
                <span class="text-gray-400">Daftar Peserta</span>
            </nav>
        </div>
    </div>
    ...
    <div class="z-50 p-6 mb-6 bg-white rounded-sm shadow-lg font-heading">
        <div class="max-w-6xl mx-auto">
            ...
            <div class="flex flex-col gap-4 mb-6 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex flex-col items-start gap-2 sm:flex-row sm:items-center sm:gap-3">
                    <form action="{{ route('peserta.create') }}">
                        @csrf
            ...
            <!-- Search Bar - Diperbesar dan Ditonjolkan -->
            <div class="mb-6">
                <form id="searchForm" method="GET" action="" class="w-full">
                    <div class="relative">
                        ...
                        <input
                            ...
                            class="w-full py-4 pr-4 text-base font-medium placeholder-gray-400 transition-all duration-200 bg-white border-2 border-gray-300 shadow-sm pl-14 rounded-xl focus:outline-none focus:ring-4 dark:bg-white dark:text-gray-900 dark:border-gray-300 focus:ring-purple-500/20 focus:border-purple-500 hover:border-purple-300"
                        >
                    </div>
                </form>
            </div>
            ...
            <!-- Filter & Controls -->
            <form method="GET" action="{{ route('peserta.index') }}" id="filterForm" class="space-y-4">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3">
                    ...
                    <div class="space-y-2">
                        ...
                        <a href="{{ route('peserta.index') }}" class="w-full inline-flex items-center justify-center px-4 py-2.5 text-sm font-medium text-gray-700 bg-gray-50 border border-gray-300 rounded-lg hover:bg-gray-100 hover:border-gray-400 focus:ring-2 focus:ring-gray-500 focus:outline-none transition-all duration-200">
            ...
            <div class="pt-4 mt-4 border-t border-gray-200">
                <div class="flex flex-col gap-3 text-sm text-gray-600 md:flex-row md:items-center md:justify-between">
                    ...
                    <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4">
                        <span>
                            Menampilkan
                            <span class="font-medium text-gray-900">{{ $participants->firstItem() ?? 0 }}–{{ $participants->lastItem() ?? 0 }}</span>
                            dari
                            <span class="font-medium text-gray-900">{{ $participants->total() }}</span> data
                        </span>
                    </div>
                    ...
                    @if ($participants->count())
                        <div class="text-gray-500">
                            Terakhir diperbarui:
                            <span class="font-medium">{{ $participants->first()->updated_at->diffForHumans() }}</span>
                        </div>
                    @endif
            ...
            <table class="min-w-full bg-white rounded shadow-md">
                <thead>
                    <tr class="text-sm font-semibold text-gray-700 bg-gray-100 font-heading">
                        ...
                        <th class="px-4 py-3 text-left uppercase whitespace-nowrap">Nama Peserta</th>
                ...
                <tbody>
                    @forelse ($participants as $p )
                        <tr class="text-sm text-gray-700 border-t hover:bg-gray-50">
                            <td class="px-4 py-3 text-left whitespace-nowrap">{{ $loop->iteration + ($participants->currentPage() - 1) * $participants->perPage() }}</td>
                            ...
                            <td class="px-4 py-3 text-left whitespace-nowrap">
                                <div class="flex space-x-2 justify-left">
                                    ...
                                    {{-- Tombol Edit --}}
                                    <form action="{{ route('peserta.edit', $p) }}" method="GET" class="inline-block">
                                        @csrf
                                    ...
                                    {{-- Tombol Hapus --}}
                                    <form action="{{ route('peserta.destroy', $p) }}" method="POST" class="inline-block" id="deleteForm-{{$p->id}}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="confirmDelete({{ $p->id }})" class="text-red-600 hover:text-red-800" title="Hapus">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none"
                                                viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <tr class="text-sm text-gray-500">
                            <td colspan="7" class="px-4 py-3 text-center">Tidak ada data peserta yang ditemukan.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="mt-4">
            @include('components.paginations', ['data' => $participants])
        </div>
    </div>
</div>
